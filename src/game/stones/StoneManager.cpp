	...
	draggedStone(NULL)
{
	...
	settings->Read(_T("General/MoveAllStones"), &moveAllStonesInRow, true);
}
...
StoneManager::~StoneManager()
{
	// Store settings
	Settings* settings = Settings::getInstance();
	settings->Write(_T("General/MoveAllStones"), moveAllStonesInRow);
...
void StoneManager::createStones(wxPanel* heap)
{
...
std::vector<Gamestone*>& StoneManager::getStones()
{
	return gamestones;
}
