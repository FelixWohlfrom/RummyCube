...
class AIPlayer: public OpponentPlayer
{
	public:
		 ...
		 */
		AIPlayer(int number, StoneManager& stoneManager, int initialStonesLeftToTake = 0, int sumInFirstRound = 0, bool stonesInOneRow = false);
	...
	private:
		 ...
		 *
		 * @param sameColor If true rows have to be in same color, otherwise rows have to be with same number
		 */
